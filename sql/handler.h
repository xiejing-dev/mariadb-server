...
public:
  ...
  virtual int init_parallel_scan(size_t n_threads) __attribute__ ((warn_unused_result))
  {
    return -1; // OLEGS: return more meaningful error
  }

  /* Call from the master thread to get context data for each worker */
  virtual Parallel_scan::Worker_ctx *get_worker_context(size_t worker_idx)
  {
    return nullptr;
  }
  ...
  /* 
    ...
    to this worker.  
  */
  virtual int pscan_init_worker(Parallel_scan::Worker_ctx *ctx)
