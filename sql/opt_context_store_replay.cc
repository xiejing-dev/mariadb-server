...
static void store_system_variables(THD *thd, struct sys_vars_ctx *ctx)
{
  ...
  store_optimizer_costs("heap", &heap_optimizer_costs, *script);
  store_optimizer_costs("temp_table", &tmp_table_optimizer_costs, *script);
    ...
    */
    ...
    if (is_base_table(tbl))
    {
      const LEX_CSTRING *engine_name=
          hton_name(tbl->table->file->partition_ht());
      if (!my_hash_search(&storage_engine_hash, (uchar *) engine_name->str,
                          engine_name->length))
      {
        if (my_hash_insert(&storage_engine_hash, (uchar *) engine_name))
        {
          res= true; // OOM
          break;
        }
